...
* File Header
Emacs 30+ warns when an init file lacks a =lexical-binding= cookie, and the
cookie only counts on the first line, so this block must stay first.
...
** Following URLs
*** Reference
Terminal output is plain text, so a URL in it is not a link. Two things make it
one:

| Key       | Action                                                      |
|-----------+-------------------------------------------------------------|
| ~C-c C-o~ | Open the URL at point, in a live terminal or in copy mode   |
| ~C-c C-t~ | Toggle copy mode, which also highlights every URL on screen |
| ~C-c RET~ | Open a highlighted URL (from ~goto-address-mode~)           |
| ~mouse-2~ | Open a highlighted URL by clicking                          |

~C-c C-o~ needs nothing highlighted — it reads the URL out of the raw text at
point — so it works in a live, scrolling terminal. The highlighting is for
finding links you have not already put point on.

*** Why highlighting is limited to copy mode
~goto-address-mode~ attaches overlays through ~jit-lock~, which works in vterm.
The problem is that vterm rewrites the buffer on every redraw, destroying those
overlays and making continuous refontification wasted work on a busy terminal.
Copy mode freezes the buffer, so the overlays survive — and that is where you
are when hunting for a link anyway. Leaving copy mode turns the mode back off,
which removes the overlays it added.

*** Why the key is bound in two maps
~vterm--enter-copy-mode~ calls ~(use-local-map nil)~, deliberately, so the
buffer behaves like ~fundamental-mode~ while you copy from it. That means the
local map — and with it ~vterm-mode-map~ — is out of the picture entirely while
copy mode is active, and only ~vterm-copy-mode-map~ remains. A binding in
~vterm-mode-map~ alone therefore works in a live terminal and is *undefined* in
copy mode, which is the wrong way round for a link-following command.

Outside copy mode the binding reaches Claude Code buffers through inheritance:
~claude-code.el~ builds its vterm keymap with ~vterm-mode-map~ as the parent.

*** Wrapped URLs
A URL wider than the window is split, and neither ~C-c C-o~ nor the highlighting
crosses a line break — point on the first half yields a truncated link that
looks like it worked, which is worse than no link at all.

Where the split came from decides whether it can be repaired:

- *Wrapped by the terminal.* vterm marks the break with a ~vterm-line-wrap~
  property — a "fake" newline, inserted only for rendering.
  ~vterm-copy-mode-remove-fake-newlines~ strips those on entering copy mode and
  puts them back on leaving, so the URL is whole while you are looking at it and
  the scrollback is unchanged afterwards. The direction matters, so it is worth
  naming where it was checked: ~vterm--enter-copy-mode~ calls
  ~vterm--remove-fake-newlines~, and ~vterm--exit-copy-mode~ calls
  ~vterm--reinsert-fake-newlines~, in emacs-libvterm as vendored here. vterm's
  own documentation recommends the option for ~isearch~ too.
- *Wrapped by the program.* If Claude emits its own newline mid-URL, that
  character is real and nothing can distinguish it from intended text. Widening
  the window before the output arrives is the only workaround.

*** Notes
- Disabling ~goto-address-mode~ unfontifies the whole buffer, and
  ~vterm-max-scrollback~ is large here, so leaving copy mode scans a long buffer
  for overlays. Cheap in practice — there are only ever a handful — but worth
  knowing, since avoiding wasted overlay work is why the mode is limited to copy
  mode in the first place.
- The tests behind this section are not committed; see issue #36.
...
*Export these variables from a login-shell file* (~~/.zprofile~ or ~~/.zshenv~
for zsh, ~~/.bash_profile~ for bash), not from ~~/.zshrc~ or ~~/.bashrc~. The
subshell runs with ~exec-path-from-shell-arguments~, which is ~("-l")~ — a
non-interactive login shell, chosen for startup speed — and a non-interactive
shell never reads the interactive rc. A variable exported only there is visible
in every terminal and yet never reaches Emacs.
...
*** Caveats
- The two halves can diverge. The buffer rename happens first, because it is
  the half that fails detectably — if the name is taken, nothing is sent. But
  ~/rename~ is a request typed into the terminal with no reply to wait for, so
  if Claude rejects it the buffer still ends up renamed. The session transcript
  shows what actually happened.
- Sending starts with an ~ESC~, because ~claude-code--term-send-string~ appends
  to whatever is already in Claude's input line: without it, a half-typed prompt
  would turn ~/rename~ into ordinary text and submit the lot. The same ~ESC~
  interrupts a turn that is still running, so renaming mid-response cancels it.
- Buffer names whose directory portion contains a colon — a TRAMP
  ~default-directory~, say — cannot be split reliably by the pattern
  ~claude-code.el~ uses, so renaming them is refused rather than risking a
  truncated name that no longer resolves.
...
*** Naming a session at creation
Starting a second instance in a project prompts for a name — =Instances already
running for ~/foo/ (existing: default), new instance name:= — and that name has
only ever been an Emacs one. The session itself kept whatever Claude called it,
so the buffer and the resume picker disagreed from birth, and ~C-c C N~ had to
be run immediately afterwards to reconcile them.

That name is now given to the session too, automatically. It applies to any
instance whose name is not the unprompted default, so ~C-c C c~ on a second
instance and ~C-c C i~ both take effect, while a project's first session — named
=default= without asking — is left alone.

*** Resuming: the buffer takes the session's name
Resuming runs the other way round. You pick a conversation from Claude's list,
so the session's name is the one that matters and the buffer should follow it —
the reverse of creating an instance, where the name you typed is the one that
matters.

Claude prints the session name at the right end of the divider above its prompt,
so it can be read back out of the buffer. Polling until a name appears also
solves a problem the readiness check cannot: the resume picker draws a border
too, and cannot be told apart from an input box, but it does not display a
session name — so waiting for a name to scrape waits past the picker without
having to recognise it.

A session that was never named shows none, and its buffer is left alone. So is
one whose name cannot be a buffer instance name, or would collide with an
existing buffer; both are reported rather than forced.

*** Waiting for the session to be ready
~claude-code-start-hook~ runs immediately after the process is spawned, long
before Claude has drawn its interface, so sending at that point would put the
command nowhere. Rather than guess a delay, this waits for evidence: Claude
renders an input box whose border is a run of ~─~, redrawn at the bottom of the
buffer, so its presence near ~point-max~ means the interface is up and typed
text will land.

Polling stops after ~my/claude-code-session-ready-timeout~ and reports rather
than sending into a terminal that never became ready. A session renamed by hand
while waiting is left as it is.
...
The negative fixtures carry the weight: this code decides when to type into a
live terminal, and every mistake it has made looked reasonable. A trust dialog
taken for the composer, a line of prose taken for a session name, the shell's
own ~-c~ taken for a resume — each has a test named for what it protects.
