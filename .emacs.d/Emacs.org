...
| ~C-c C-t~ | Enter copy mode, which also highlights every URL on screen |
...
- *Wrapped by the terminal.* vterm marks the break with a ~vterm-line-wrap~
  ...
  puts them back on leaving, so the URL is whole while you are looking at it.
  Enabled below; vterm's own documentation recommends it for ~isearch~ too.
