...
| Key       | Action                                                     |
|-----------+------------------------------------------------------------|
| ~C-c C-o~ | Open the URL at point, whether or not copy mode is on      |
...
This reaches Claude Code buffers too: ~claude-code.el~ builds its vterm keymap
with ~vterm-mode-map~ as the parent, so bindings added there are inherited.

*** Caveat: wrapped URLs
A URL long enough to wrap is broken by a real newline before Emacs ever sees it,
and nothing here reassembles it:

#+begin_example
  https://github.com/rmorison/.dotfiles/
  pull/35
#+end_example

Point on that yields ~https://github.com/rmorison/.dotfiles/~ — a truncated link
that looks like it worked, which is worse than no link at all. Both ~C-c C-o~ and
the highlighting share the limitation, since neither crosses a line break.
Widening the window before the output arrives is the only real workaround.
