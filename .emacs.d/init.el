...
sits above whatever status lines Claude draws beneath it.")
  ...
  (defcustom my/claude-code-ready-regexp
    "\\(?:mode on\\|for agents\\|for shortcuts\\|esc to interrupt\\)"
    "Match chrome that only Claude's composer shows.
Deliberately not the `❯' pointer: that is the CLI's selection cursor,
drawn on the focused row of any picker or dialog, all of which are
bordered.  Matching it would declare a folder-trust prompt ready and
type into it -- and that dialog focuses its cancel option, so the
injected return would exit.  The mode line beneath the composer is not
drawn by those dialogs.  Scraped from rendered output, so it is
adjustable here when the interface moves."
    :type 'regexp
    :group 'my/claude-code-naming)

  (defun my/claude-code--tail (buffer chars)
    "Return the last CHARS characters of BUFFER."
    (with-current-buffer buffer
      (buffer-substring-no-properties
       (max (point-min) (- (point-max) chars)) (point-max))))

  (defun my/claude-code--session-ready-p (buffer)
    "Non-nil once BUFFER shows Claude's composer, ready for typed text.
Requires a box border *and* composer chrome: a border alone is drawn by
dialogs that must not be typed into."
    (and (buffer-live-p buffer)
         (let ((tail (my/claude-code--tail buffer my/claude-code--ready-tail)))
           (and (string-match-p "─" tail)
...
ahead of the redraw is waited out rather than taken for the answer."
  ...
  (defvar-local my/claude-code--naming-timer nil
    "Pending naming poll for this buffer, so typing can cancel it.")

  (defun my/claude-code--cancel-naming ()
    "Abandon a pending naming poll once the session is being used.
Readiness stays true while Claude is responding, so a late escape would
interrupt a turn deliberately started and discard what was typed."
    (when my/claude-code--naming-timer
...
direction that types into the resume picker."
    (when-let* ((proc (get-buffer-process (or buffer (current-buffer)))))
      (let* ((command (mapconcat #'identity (process-command proc) " "))
             ;; The process is spawned through `sh -c "... exec claude ARGS"',
             ;; so match only from the exec onwards. Scanning the whole line
             ;; would see the shell's own `-c' and call every session a resume.
             (args (if (string-match "\\bexec\\b" command)
                       (substring command (match-end 0))
                     command)))
        ;; Long, short and =VALUE forms, since a miss falls through to the
        ;; create branch -- the one that types into the picker.
        (string-match-p
         "\\(?:^\\| \\)\\(?:--\\(?:resume\\|continue\\)\\(?:[ =]\\|$\\)\\|-[rc]\\(?: \\|$\\)\\)"
         args))))
  ...
  (defconst my/claude-code--session-name-regexp
    "^─+ \\([^ \n─][^\n─]*?\\) ─+\n❯"
    "Match the session name on the divider directly above the composer.
Anchoring to the composer prompt rather than scanning for any rule is
what keeps prose out: a line like `─── Summary of changes ───' in
Claude's own output is structurally identical to a name-bearing divider,
and only its position distinguishes them.  The captured name may not
itself contain `─', which rejects a multi-segment status line.")

  (defun my/claude-code--scraped-session-name (buffer)
    "Return the session name Claude is displaying in BUFFER, or nil.
Searches backward, so the newest composer wins when a resumed transcript
has replayed several dividers into view."
    (when (buffer-live-p buffer)
      (with-current-buffer buffer
        (save-excursion
          (goto-char (point-max))
    ...
    (when (buffer-live-p buffer)
      (let ((name (my/claude-code--scraped-session-name buffer))
        ...
        (cond
         ...
         (t (my/claude-code--poll-again
             #'my/claude-code--adopt-name-when-ready buffer deadline))))))
      ...
      (cond
       ...
       ((my/claude-code--resuming-p (current-buffer))
        (when my/claude-code-adopt-session-name
          ;; The resume picker is driven by a human, so the clock cannot start
          ;; at spawn: choosing a session takes as long as it takes.
          (my/claude-code--adopt-name-when-ready
           (current-buffer)
           (+ (float-time) my/claude-code-resume-ready-timeout))))
       (my/claude-code-name-new-sessions
        (my/claude-code--name-session-when-ready (current-buffer) deadline)))))

  (with-eval-after-load 'claude-code
    ;; Guarded like the sibling features in this file: `run-hooks' in
    ;; `claude-code--start' is bare and the buffer is displayed after it, so an
    ;; error here would leave a session created but never shown.
    (if (fboundp 'claude-code--extract-instance-name-from-buffer-name)
        (add-hook 'claude-code-start-hook #'my/claude-code--sync-names-on-start)
      (warn "claude-code--extract-instance-name-from-buffer-name is missing; \
