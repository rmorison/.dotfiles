...
;; Intelligent frame setup based on display width
(defun my/setup-frames ()
  "Setup frames intelligently based on display width.
For displays <= 1920px wide: maximize single frame.
For displays > 1920px wide: create two side-by-side frames on the widest display."
  (let* ((monitors (display-monitor-attributes-list))
         (widest-monitor (car (sort monitors
                                   (lambda (a b)
                                     (> (nth 3 (assoc 'geometry a))
                                        (nth 3 (assoc 'geometry b)))))))
         (geometry (assoc 'geometry widest-monitor))
         (monitor-x (nth 1 geometry))
         (monitor-y (nth 2 geometry))
         (monitor-width (nth 3 geometry))
         (monitor-height (nth 4 geometry)))
    (if (<= monitor-width 1920)
        ;; Standard display: maximize single frame
        (set-frame-parameter nil 'fullscreen 'maximized)
      ;; Wide display: create two side-by-side frames on the widest monitor
      (let* ((frame-width (/ monitor-width 2))
             (frame-height monitor-height)
             (left-frame (selected-frame))
             (right-frame (make-frame)))
        ;; Position left frame on widest monitor
        ...
        ;; Position right frame on widest monitor
        (set-frame-position right-frame (+ monitor-x frame-width) monitor-y)
